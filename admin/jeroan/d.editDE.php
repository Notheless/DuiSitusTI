...
//insert
	...
	$sql = "INSERT INTO `dosen`(`ID`, `Nama`, `Email`, `HP`, `Telp`, `Alamat`, `Pendidikan`, `Bidang`) VALUES (".$id.",'".$Nama."','".$Email."','".$HP."','".$Telp."','".$Alamat."','".$Pendidikan."','".$Bidang."');";
	...
	foreach($Pengajaran as $input){
		if(strcmp($input,"")){
			$sql = "INSERT INTO `detaildosen`(`IDDosen`, `IDCatagory`, `Isi`) VALUES (".$id.",1,'".$input."')";
			$conn->query($sql);
		}
	}
	foreach($pengabdian as $input){
		if(strcmp($input,"")){
			$sql = "INSERT INTO `detaildosen`(`IDDosen`, `IDCatagory`, `Isi`) VALUES (".$id.",3,'".$input."')";
			$conn->query($sql);
		}
	}
	foreach($penunjang as $input){
		if(strcmp($input,"")){
			$sql = "INSERT INTO `detaildosen`(`IDDosen`, `IDCatagory`, `Isi`) VALUES (".$id.",4,'".$input."')";
			$conn->query($sql);
		}
	}
	foreach($Penelitian as $input){
		if(strcmp($input,"")){
			$sql = "INSERT INTO `detaildosen`(`IDDosen`, `IDCatagory`, `Isi`) VALUES (".$id.",2,'".$input."')";
			$conn->query($sql);
		}
	}
	echo "

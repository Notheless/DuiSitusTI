        <div class="left-sidebar-container">
            <div class="left-sidebar-style">
                <!-- search -->
                <form action="Posting.php">
                    <input class="search-input" type="text" name="search" placeholder="Cari" />
                    <input class="search-submit" type="submit" value="🔍"/>
                </form>
                <!-- tags -->
                <h1 class="left-nav-header">TAGS</h1>
                <div class="tags-container">
                    <div class="tags-container-hover"></div>
                    <a href="posting.php?tags=1">Kecendekiawanan</a>
                    <a href="posting.php?tags=2">Kerjasama</a>
                    <a href="posting.php?tags=3">Laboratorium</a>
                    <a href="posting.php?tags=4">Lulusan</a>
                    <a href="posting.php?tags=5">Mahasiswa</a>
                    <a href="posting.php?tags=6">Monev-Mutu</a>
                    <a href="posting.php?tags=7">Pendidikan</a>
                    <a href="posting.php?tags=8">Penelitian</a>
                    <a href="posting.php?tags=9">Pengabdian</a>
                    <a href="posting.php?tags=10">Prestasi</a>
                    <a href="posting.php?tags=11">Sarpras</a>
                    <a href="posting.php?tags=12">Seminar</a>
                    <a href="posting.php?tags=13">Standar 1</a>
                    <a href="posting.php?tags=14">Standar 2</a>
                    <a href="posting.php?tags=15">Standar 3</a>
                    <a href="posting.php?tags=16">Standar 4</a>
                    <a href="posting.php?tags=17">Standar 5</a>
                    <a href="posting.php?tags=18">Standar 6</a>
                    <a href="posting.php?tags=19">Standar 7</a>
                    <a href="posting.php?tags=20">Tridarma Lainnya</a>
                </div>
                <!-- subscribe -->
                <div class="subscribe-container">
                    <div class="subscribe-header">
                        <li class="subscribe-icon"><img class="subscribe-icon" src="img/mail.png" height="40px" width="60px" /></li>
                        <p>Subscribe <span>untuk mengikuti perkembangan terbaru kami!</span></p>
                    </div>
                    <input class="subscribe-input" type="email" name="email" placeholder="Alamat surel anda" />
                    <div class="subscribe-succ" id="subscribe-animation"><p>Subscribe berhasil!</p><div class="lowbar"></div></div>
                    <button class="subscribe-submit" onclick="showSubscribeAnimation()" id="subscribe-button">SUBSCRIBE</button>
                    <!-- <input class="subscribe-submit" type="submit" value="SUBSCRIBE" /> -->
                </div>
                <!-- link -->
                <h2 class="left-sidebar">LINKS</h2>
                <a class="left-sidebar" href="https://siat.unpad.ac.id/index.php/login">Sistem Informasi Akademik Terpadu (SIAT)<div></div></a>
                <a class="left-sidebar" href="http://informatika.unpad.ac.id/dreamspark/">Microsoft Imagine Unpad<div></div></a>
                <a class="left-sidebar" href="http://cdc.unpad.ac.id/">Lowongan Kerja dan Karir<div></div></a>
                <a class="left-sidebar" href="http://kandaga.unpad.ac.id/">Unpad Repositories<div></div></a>
                <a class="left-sidebar" href="https://staffs.unpad.ac.id/login">Sistem Informasi Staff<div></div></a>
                <a class="left-sidebar" href="http://blogs.unpad.ac.id/informatika/">BLOGS Informatika<div></div></a>
                <a class="left-sidebar" href="http://amadi.unpad.ac.id/">AMADI Research<div></div></a>
                <a class="left-sidebar" href="http://pak.ristekdikti.go.id/portal/">Sistem Penilaian Angka Kredit Dosen<div></div></a>
                <!-- info -->
                <h2 class="left-sidebar">CONTACT US ON</h2>
                <p>Kampus UNPAD Jatinangor<br />Jalan Raya Bandung-Sumedang KM21<br />Jatinangor, 45363<br />Gedung Ilmu Komputer (D PPBS)</p>
                <p>Telepon : 6222-7798983</p>
                <p>Fax : 6222-7794545</p>
                <p>Message : 6281-1234734</p>
                <p>SMS : 6281-22323328</p>
                <p>Email : Informatika@unpad.ac.id</p>
                <p>Website : Informatika.unpad.ac.id</p>
                <p>© 2017 TI UNPAD</p>
            </div>
            <div class="end-of-left-nav"></div>
        </div>
        <script type="text/javascript" src="js/subscribe-anim.js"></script>